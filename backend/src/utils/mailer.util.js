...
const sendPasswordResetEmail = async (to, nombre, token, isPatient = true) => {
  // If isPatient is true, it goes to the patient portal, else the staff portal
  const baseUrl = isPatient ? process.env.PORTAL_URL : process.env.FRONTEND_URL;
  const url   = `${baseUrl}/reset-password/${token}`;
  
  await transporter.sendMail({
    from:    process.env.MAIL_FROM,
    to,
    subject: 'Restablecer contraseña — Consultorio Padre Pio',
    html: `
<!DOCTYPE html>
<html lang="es">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
        ...
        <table width="520" cellpadding="0" cellspacing="0"
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              ...
              <p style="margin:0 0 20px;font-size:14px;color:#475569;line-height:1.6;">
                Recibimos una solicitud para restablecer tu contraseña. Si fuiste tú, haz clic en el siguiente botón para asignar una nueva contraseña.
              </p>
              <table cellpadding="0" cellspacing="0" style="margin:0 auto 28px;">
                <tr>
                  <td style="background:#8BC63F;border-radius:8px;">
...
module.exports = {
  ...
  sendFamiliarActivadoEmail, sendAppointmentConfirmationEmail, sendPasswordResetEmail
};
